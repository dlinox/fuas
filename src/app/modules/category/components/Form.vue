        ...
        <v-card-item class="border-t border-b">
          <v-row>
            ...
            <v-col cols="12">
              <v-select
                v-model="form.type"
                :items="['ASISTENCIAL', 'ADMINISTRATIVO']"
                label="Tipo"
                :rules="CategoryRules.type"
              />
            </v-col>
...
import { _saveItem } from "@/app/modules/category/services/category.services";

const emit = defineEmits(["onSuccess"]);

const props = defineProps({
  formState: {
    type: Object as () => Partial<CategoryDTO>,
    default: () => ({}),
  },
});
